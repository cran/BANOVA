BANOVA.BinNormal <-
function(l1_formula = 'NA', l2_formula = 'NA', data, id, num_trails, burnin, sample, thin, jags){
  ...
  if (class(num_trails) != 'integer') stop('The number of trails should be integers! Might use as.integer()')
  ...
  N <- num_trails
  if (length(N) == 1) N <- rep(num_trails, n) 
  if (length(N) != n) stop('The length of num_trails must be equal to the number of observations!')
  if (sum(y > num_trails) > 0) stop('The number of trails is less than observations!')
  JAGS.model <- JAGSgen.binNormal(dMatrice$X, dMatrice$Z)
  JAGS.data <- dump.format(list(n = n, id = id, M = num_id, y = dMatrice$y, X = dMatrice$X, Z = dMatrice$Z, N = N))
